  ...
  if (isset($_POST['bouton'])) { // si je clique sur mon input de nom bouton
    ...
    if (!empty($_POST['login']) AND !empty($_POST['email']) AND !empty($_POST['mdp'])){// Si les inputs login,email et mdp ne sont pas vide
      if (filter_var($email, FILTER_VALIDATE_EMAIL)) {// Si l'email est une email
        ...
        else {//sinon je renvois vers un code http 401 suivi d'un message d'erreu
          $erreur = 'Votre login est déja utilisé';
        }
      }
      else{ //sinon je renvois vers un code http 401 suivi d'un message d'erre
        $erreur = "Votre adresse mail n'est pas valide!";
      }

    }
    else{//sinon je renvois vers un code http 401 suivi d'un message d'err
...
// FIN_INSCRIPTION
  ...
  <html>
    ...
    <body>
      ...
      <div align='center'>
        <h2>Inscription</h2>
        <h4>En tant que : Candidat</h4>
          <br><br>
        <form action="/signup_candidat" method="post">
