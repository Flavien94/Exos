...
<?php
	...
	<table class="calendrier">
		<caption><?php echo '<a href="calendar.php?m='.$mois_avant.'&amp;y='.$annee_avant.'"><<</a>  '.$m[$numero_mois].' '.$annee.'  <a href="calendar.php?m='.$mois_apres.'&amp;y='.$annee_apres.'">>></a>'; ?></caption>
		...
		echo '<tr>';
			...
			for($i = 1 ; $i <= 7 - $numero_jour1er; $i++) {
				...
				if (in_array($i, $event)) {
					...
					echo '"><a href="php/evenement.php?d='.$i.'/'.$numero_mois.'/'.$annee.'" class="info">'.$i.'<span>'.afficheEvent($i, $event).'</span></a></div></td>';
				} else {
					echo '<td ';
			...
			for($colone = 0 ; $colone < 7 ; $colone++) {
				if($i <= date('t', $timestamp))	{
					...
					if (in_array($i, $event)) {
						...
						echo '"><a href="php/evenement.php?d='.$i.'/'.$numero_mois.'/'.$annee.'" class="info">'.$i.'<span>'.afficheEvent($i, $event).'</span></a></td>';
					} else {
						echo '<td ';
